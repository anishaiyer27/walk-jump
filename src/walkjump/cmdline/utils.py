...
def instantiate_model_for_sample_mode(
    ...
    print("Config received:", sample_mode_model_cfg)
    print(
    ...
    elif isinstance(model, NoiseEnergyModel) and sample_mode_model_cfg.denoise_path is None:
        print(
            "[instantiate_model_for_sample_mode] (model.noise_ebm)",
            _LOG_MSG_INSTANTIATE_MODEL.format(
                model_type="noise_ebm", checkpoint_path=sample_mode_model_cfg.checkpoint_path
            ),
        )
        model.noise_ebm = NoiseEnergyModel.load_from_checkpoint(sample_mode_model_cfg.checkpoint_path)
        model.noise_ebm.eval()
        model.noise_ebm.training = False
